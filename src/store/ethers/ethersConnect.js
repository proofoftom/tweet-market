...
export function web3Ok() {
  ...
  const ok =
    web3 && web3.isConnected && web3.isConnected() && web3.currentProvider;
  return ok;
}
...
async function _getWallet() {
  if (!walletFns[PROVIDER_TYPE])
    throw new Error(MSGS.INVALID_PROVIDER + " " + PROVIDER_TYPE);
...
export async function startProviderWatcher() {
  ...
  async function updateProvider() {
    ...
    if (!createProvider[PROVIDER_TYPE])
      throw new Error(msgs.ethers.invalidProvider + " " + PROVIDER_TYPE);
    try {
      // try to update provider
      web3Provider = await pTimeout(
        createProvider[PROVIDER_TYPE](),
        WEB3_TIMEOUT
      );
      try {
        await _getWallet();
      ...
      if (LOG_TRANSACTIONS.length) {
        for (let log of LOG_TRANSACTIONS) {
          web3Provider.on(log, msg =>
            console.log(
              'Example ethereum event log. Configure "LOG_TRANSACTIONS" array for more or less',
...
export async function getWalletAddress() {
  const addr =
    (userWallet.getAddress && (await userWallet.getAddress())) ||
    userWallet.address;
  return addr;
}

export async function getWalletBalance(address) {
  const balance = utils.formatEther(await web3Provider.getBalance(address));
  return balance;
}
